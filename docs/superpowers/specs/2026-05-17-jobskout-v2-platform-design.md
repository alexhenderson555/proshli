# JobSkout V2 — Platform Design

**Дата:** 2026-05-17
**Статус:** Design (требует ревью)
**Автор:** Alex (с помощью ИИ-ассистента)
**Базируется на:** существующем JobSkout MVP (FastAPI + Next.js + TG-бот)

---

## 1. TL;DR

JobSkout V2 — крупная итерация существующего MVP до уровня премиум job-search платформы для IT-аудитории РФ и СНГ. Главные дифференциаторы относительно HH/Habr Career:
...
| Слой | Технология | Обоснование |
|---|---|---|
| API | **FastAPI** (Python 3.12) | Уже используется в JobSkout MVP, AI/ML экосистема в Python мощнее всего |
...
| Auth | **JWT** + magic link + OAuth (Yandex, Google) | Уже частично в JobSkout |
...
1. **Название продукта** — оставляем JobSkout или ребрендинг под более сильное имя? (Sondage в close beta.)
2. **Юрисдикция для AI-платежей** — оформление армянского/грузинского ИП на год 2 для прямой работы с Anthropic/OpenAI без посредников.
3. **Дизайнер на парт-тайм vs дизайн-система shadcn** — на MVP возможно справиться без дизайнера, но для «10/10 premium» лучше привлечь хотя бы на 1-2 спринта.
4. **Источники Telegram-каналов** — нужен curated список 30-50 каналов с приоритетом по качеству вакансий.
5. **B2B-партнёрство с HH** — есть ли смысл подавать заявку на программу интеграторов уже на MVP-этапе.
