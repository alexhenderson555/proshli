...
// Категории Tailwind мапятся на ветки tokens.json **вручную** — это намеренно.
// Если завтра в tokens.json появится `breakpoints` или `zIndex`, дописать ему
// строку в preset надо явно: иногда новая категория не должна попадать в
// Tailwind (например, internal-only переменные). Auto-map скрыл бы это решение.
